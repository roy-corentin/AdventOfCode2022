...
def next_round(monkeys)
  ...
  monkeys.each do |id, monkey|
    ...
    new_items = monkey['items'].map(&monkey['operation']).map { |object| object / 3 }
...
puts multiply_two_most_active_monkey(open_file('input.txt'), 20) # rigth answer 58322
